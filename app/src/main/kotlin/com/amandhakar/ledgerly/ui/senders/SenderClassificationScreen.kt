...
private val CLASSIFICATIONS = listOf(
    ...
    SenderType.NOT_FINANCIAL to false,
)

private fun SenderType.label(): String = if (this == SenderType.NOT_FINANCIAL) "NOT FINANCIAL" else name
 ...
 *
 * NOT_FINANCIAL (found in live use, not the corpus): the pre-filter's TRANSACTION default is
 ...
 * honest label without changing behavior. It's a distinct enum value from UNKNOWN (the
 * never-classified default) rather than reusing it — [SenderClassificationViewModel]'s pending
 * list keys on exactly that distinction to know a sender was actually classified.
 */
@Composable
